    ...
    <div x-data="workflowBuilder()" x-init="initBuilder()"
        ...
        <div class="max-w-full px-8">
            ...
            <div class="mb-6 flex items-center gap-2 text-sm">
                <a href="{{ route('workflowsBuilder') }}" class="text-slate-500 dark:text-gray-400 hover:text-slate-700 dark:hover:text-gray-300 transition-colors">Manajemen Alur Kerja</a>
                            ...
                            <template x-for="(doc, index) in documents" :key="doc.tempId || doc.id">
                                <div class="bg-white dark:bg-kinetic-card border border-slate-200 dark:border-kinetic-border rounded-xl p-6 hover:shadow-lg transition-all group relative">
                                    ...
                                    <div class="mt-2 pt-4 border-t border-slate-200 dark:border-kinetic-border flex items-center justify-between">
                                        <span class="text-xs font-bold uppercase tracking-widest text-teal-600">Wajib Dilampirkan</span>
                                    </div>
                                </div>
                            </template>
    ...
    <script>
        function workflowBuilder() {
            return {
                ...
                workflow: null,
                activeTab: @js(Auth::user()->unit->level === 'Organisasi' ? 'alur' : 'dokumen'),
                loading: true,
                saving: false,
